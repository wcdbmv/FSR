...
void iterate() {
	...
	for (size_t k = 0; /* no condition */; ++k) {
		...
		for (size_t i = 0; i < N; ++i) {
			...
			const auto im1 = i > 0 ? i - 1 : 0;
			const auto ip1 = i < N - 1 ? i + 1 : N - 1;
			for (size_t j = 0; j < N; ++j) {
				const auto zj = static_cast<float>(j) * h;
				const auto jm1 = j > 0 ? j - 1 : 0;
				const auto jp1 = j < N - 1 ? j + 1 : N - 1;
				y_k_plus_1[i][j] = (y_k[ip1][j] + y_k[im1][j] + y_k[i][jp1] + y_k[i][jm1] + f(xi, zj) / k_0 * sqr(h)) / 4;
